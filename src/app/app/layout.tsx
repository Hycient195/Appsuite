...
function AppLayout({ children }: IProps) {
  ...
  return (
    <div onClick={hideSidebarOnMobile} className={`grid bg-white fixed w-full grid-cols-1 ${!isSidebarHidden && "lg:grid-cols-[max-content_1fr]"} font-lexend`}>
      <div className={`${isMobileSidebarHidden && "hidden"} slide-wrapper lg:hidden h-[100dvh] w-screen left-0 bg-gray-700/30 backdrop-blur-s absolute z-[5] animate-screen-slide-in-right`} />
      <AnimatePresence>
        {
          // (!isSidebarHidden || !isMobileSidebarHidden)
          ( (isMobileSidebarHidden && typeof window !== "undefined" && window.innerWidth <= 1024) ? !isMobileSidebarHidden  : !isSidebarHidden)
          && (
            <motion.div
              initial={{ x: "-100%" }}
              animate={{ x: "0" }}
      ...
      <div className="w-full relative bg-slate-100 !overflow-scroll min-h-[100dvh] max-h-[100dvh] ">
        ...
        { !isNotificationModalHidden && <div onClick={() => dispatch(sharedSlice.actions.hideNotificationModal())} className="animate-fade-in w-[calc(100vw-clamp(270px,20vw,280px))] z-[10] bg-black/20 cursor-pointer backdrop-blur-s fixed top-0 right-0 h-full p-3 md:p-4 lg:p-5">
          <NotificationModal />
        </div> }
