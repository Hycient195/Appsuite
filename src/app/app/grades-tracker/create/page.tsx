...
export default function CreateCGPATrackerSheet() {
  ...
  const handleCreateFile = (e: FormEvent) => {
    e.preventDefault();
    console.log("Fired")
    const pageDefault = [{ ...defaultPage, title: formData.fileName, cgpaScale: formData.gpaScale }].map((page) => generateCSVData(page)).join('\n,,,,\n,,,,\n');
      ...
      <div className="bg-whit border border-zinc-200 rounded-md min-h-full flex items-center justify-center">
        <form onSubmit={handleCreateFile} className="max-w-lg mx-auto w-full border border-zinc-200 -translate-y-[clamp(10px,3vh,100px)] rounded-lg bg-white flex gap-3 flex-col p-[clamp(16px,2%,40px)] shadow-inne">
          <h2 className="text-xl font-sans font-semibold text-center tracking-wide">Create Sheet</h2>
            <FormText labelText="File Name" labelClassName="tracking-wide"  placeholder="" value={formData.fileName} onChange={(e) => setFormData({ ...formData, fileName: e.target.value })} />
            <FormSelect labelText="CGPA Scale" value={formData.gpaScale} onChange={(e) => setFormData({ ...formData, gpaScale: e.target.value })} options={[ { text: "5-Point Scale", value: "OVER5" }, { text: "4-Point Scale", value: "OVER4"} ]} />
          ...
          <LoadingButton loading={isLoading} type="submit" className="btn bg-black text-white">Create File</LoadingButton>
        </form>
      </div>
    </main>
  )
