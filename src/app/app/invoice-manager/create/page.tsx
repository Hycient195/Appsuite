  ...
  return (
    <main className="h-full">
      <div className="bg-whit borde border-zinc-200 rounded-md min-h-full flex flex-col ">
        ...
        <div className="max-w-lg mx-auto w-full border border-zinc-200 h-max rounded-md items-center justify-center bg-white flex flex-row gap-3 p-[clamp(10px,1.5%,30px)] shadow-inne">
        ...
        <div className="">
           <h2 className="text-xl">Invoice Samples</h2>
           <div className="line" />
           <div className="flex flex-col gap-8">
            {
              Object.entries(invoiceTemplates)?.map((templateCategory, categoryIndex) => (
                <div key={`template-category-${categoryIndex}`} className="grid">
                  ...
                  <div className="overflow-x-auto h-[410px] overflow-y-visible h- grid">
                    <div className="flex  flex-row items-center min-w-max gap-2">
                      {
...
const ScaledWrapper: React.FC<ScaledWrapperProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`inline-block relative h-[380px] w-[270px] bg-test ${className}`}
      style={{
    ...
    >
      <div
        ...
        className="absolute w-[900px] z-[1] top-0 left-0 scale-[0.3]">{children}</div>
    </div>
  );
};
