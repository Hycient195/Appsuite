...
const ReceiptTracker: React.FC<{csvString: string, isLoggedIn: boolean, loadedSucessfully: boolean }> = ({ csvString, isLoggedIn, loadedSucessfully }) => {
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>, pageIndex: number, rowIndex: number) => {
    ...
    axios.patch("/api/google-drive/file-in-folder", data)
      .then((res) => {
        const pagesCopy = [ ...pages ]
        pagesCopy[pageIndex].rows[rowIndex].receipt = res.data;
        setPages(pagesCopy);
    ...
    // saveFileInFileFolder(data as any);
  };
                            ...
                            <tr style={{ fontFamily: "sans-serif"}} key={rowIndex} className={`relative group/row hover:cursor-pointer group-has-[button.remove-btn]:hover:[&_div.remove-hover]:!hidden`}>
                              ...
                              <td className="flex items-center">
                                {/* <div className="flex flex-row items-center justify-center w-full gap-2">
                                  <label htmlFor={`file-input-${pageIndex}-${rowIndex}`}>
                                ...
                                {/* <input type="file" onChange={(e) => handleUploadImage(e, pageIndex, rowIndex)} /> */}
                                {/* <input
                                  ref={(el) => {inputRefs.current.set(`${pageIndex}-${rowIndex}-amount`, el)}}
                                  ...
                                  value={splitInThousandForTextInput(row.receipt === "0" ? "" : row.receipt)}
                                  disabled={(!!row.amount&&row.amount!=="0") || row.receiptName === "BALANCE BROUGHT FORWARD"}
                                  onChange={e => handleInputChange(pageIndex, rowIndex, 'receipt', e.target.value?.replace(/[^0-9.]/g, ""))}
                                  onKeyDown={(e) => handleKeyDown(e, pageIndex, rowIndex, "amount")}
