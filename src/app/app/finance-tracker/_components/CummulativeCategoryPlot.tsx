...
const CummulativeCategoryPlot: React.FC<ICategoryTotals> = ({ categoryTotals }) => {
  const { pages } = useFinanceTrackerContext();
  const [plotType, setPlotType] = useState<string>(PlotTypes.LINE);
  ...
  const aggregateCategoryTotals = () => {
    const aggregatedData: Record<string, any>[] = [];

    pages.forEach((page, pageIndex) => {
      const pageData: Record<string, any> = { page: pageIndex + 1 };
      
      if (page.pageCategoryTotal) {
        Object.entries(page.pageCategoryTotal.debit).forEach(([category, total]) => {
          pageData[category] = total;
        });
  
        Object.entries(page.pageCategoryTotal.credit).forEach(([category, total]) => {
          pageData[category] = total;
        });
      }
  ...
  const aggregatedData = aggregateCategoryTotals();
      ...
      case PlotTypes.LINE:
        return (
          <LineChart data={data}>
            ...
            <Tooltip />
            <Legend />
            {Object.keys(data[0]).filter(key => key !== 'page').map((category, index) => (
              <Line key={index} type="monotone" dataKey={category} stroke={getColorByWord(category).color} />
            ))}
          </LineChart>
        );
      case PlotTypes.BAR:
  ...
  return (
    <details>
      ...
      <div className='mt-4'>
        ...
        <h2>Category Totals Across Pages</h2>
        <ResponsiveContainer width="100%" height={400}>
          {renderPlot(aggregatedData, 'Category Totals Across Pages') as any}
        </ResponsiveContainer>
...
export default CummulativeCategoryPlot;