...
export default function ModuleFilesHeader({ moduleName, subtitle, handleInitiateCreateFile }: IProps) {
  const handleImport = () => {
  ...
  return (
    <section className="w-full spread-out gap-3 max-md:text-center max-md:justify-center flex-wrap">
      ...
      <div className="right line-in gap-3">
        ...
        <button onClick={handleInitiateCreateFile} className="btn bg-primary text-white"><PlusIcon className="!size-5 !stroke-[2px]" /> Create New Sheet</button>
      </div>
    </section>
  )
}